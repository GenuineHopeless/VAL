...
// конструкторы
template <typename T>
vec<T>::vec() : size(0), capacity(0) {};
template <typename T>
vec<T>::vec(size_t initialCapacity) : size(0), capacity(initialCapacity) {
	if (capacity > 0) {
		data = std::make_unique<T[]>(capacity);
	}
}
template <typename T>
vec<T>::vec(std::initializer_list<T> init) : size(init.size()), capacity(init.size()) {
	if (capacity > 0) {
		data = std::make_unique<T[]>(capacity);
...
template <typename T>
vec<T>::vec(const vec& other) : size(other.size), capacity(other.capacity) {
	if (capacity > 0) {
		data = std::make_unique<T[]>(capacity);
...
template <typename T>
vec<T>::vec(vec&& other) noexcept : data(std::move(other.data)), size(other.size), capacity(other.capacity) {
	other.data = nullptr;
	other.size = 0;
	other.capacity = 0;
}
...
template<typename T>
void vec<T>::resize(size_t newSize) {
	if (newSize > capacity) {
		reallocate(newSize);
	}
...
template <typename T>
void vec<T>::reserve(size_t newCapacity) {
...
// итераторы
template <typename T>
auto vec<T>::begin() noexcept -> vec<T>::iterator {
	return data ? iterator(data.get()) : iterator();
}
template <typename T>
auto vec<T>::end() noexcept -> vec<T>::iterator {
	return data ? iterator(data.get() + size) : iterator();
}
template <typename T>
auto vec<T>::begin() const noexcept -> vec<T>::const_iterator{
	return data ? const_iterator(data.get()) : const_iterator();
}
template <typename T>
auto vec<T>::end() const noexcept -> vec<T>::const_iterator{
	return data ? const_iterator(data.get() + size) : const_iterator();
}
template <typename T>
auto vec<T>::cbegin() const noexcept -> vec<T>::const_iterator{
	return begin();
}
template <typename T>
auto vec<T>::cend() const noexcept -> vec<T>::const_iterator{
	return end();
}
